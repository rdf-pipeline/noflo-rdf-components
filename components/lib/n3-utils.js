 ...
 * asynchronous method.  
 * 
 * @param jsonld_data JSON-LD data to be loaded to the N3 store
 * 
 * @return the N3 store
 */
 ...
 *  
 * @param term the triple element to process
 *
 ...
 *
 * @return a promise to return the N3 store converted to Turtle format or a reject on error
 */
 ...
 * @param filename (optional) an optional file name to write the Turtle result into.
 *
 * @return a promise to return the N3 store converted to JSON-LD format or a reject on error
 */ 
    ...
    return new Promise(function(resolve, reject) { 
        ...
        function(json) {
            ...
            return new Promise(function(resolve, reject) { 
                ...
                return jsonld.frame(json, frame, function(err, framed) {
                    if (!_.isEmpty(err)) { 
                        return reject(errUtils.errorMessage("n3ToJsonld API was unable to frame JSON!", err));
                     }

                     // Filter out ids with blank nodes that to nothing else
                     var filterBnodeAttrs = [ 'id', '_id', '@id' ];
                     framed['@graph'] = [ ttl2Jsonld.filterBNodes(framed['@graph'][0], filterBnodeAttrs) ];
                     ...
                     fs.writeFile(filename, JSON.stringify(framed, null, 2), 
                         function(error) { 
                             if (!_.isNull(error)) { 
                                 return reject(errUtils.errorMessage("n3ToJsonld API was unable to write file " + filename + "!\n", error));
                             } 
 ...
 * 
 * @param value (require) value to be converted
 * 
 */
