...
var fRunUpdater = function( updater, updaterArgs, component, vni ) {

    console.log('javascript-wrapper fRunUpdater');

    // Have one or more parameters - get them in the right order and pass them 
    var self = this;
    var parameters = _.map( updaterArgs, function( arg ) { 
        var state = vni.inputStates( component, vni.vnid, arg);
        return state.data;
    });
   
    // Execute the updater
    var results = updater.apply( component, parameters );

    // Clear the input states we just used since we are done with them
    updaterArgs.forEach( function( arg ) { 
        vni.inputStates( component, vni.vnid, arg, undefined);
    });
