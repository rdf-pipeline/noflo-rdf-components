...
var fRunUpdater = function( updaterArgs, portData ) {

    // Have one or more parameters - get them in the right order and pass them 
    var self = this;
    var parameters = _.map( updaterArgs, function( arg ) { 
        var parameter = (_.isArray(self.rpf.data[arg]) && self.rpf.data[arg].length==1) ?
                            self.rpf.data[arg][0] : self.rpf.data[arg];
        return parameter; 
     });

     var results = this.rpf.updater.apply( this, parameters );

     if ( results ) { 
         // NOTE: replace when lower layers can do this
         self.outPorts.output.send( results )
         self.outPorts.output.disconnect();
         self.rpf.data = []; // Revisit as lower layers are implemented
     }
...
module.exports = function(nodeDef) { 
    ...
    return componentFactory( 
        _.defaults( { inPorts: _.mapObject(nodeDef.inPorts, function( port, portName ) {  

                         return _.extend({
                            ondata: function(payload, socketIndex) { 

                                 // socketIndex is only defined for addressable/multi ports.  
                                 // Set default of 0 if it is undefined - there will only be one socketIndex non-multiports anyway.
                                 socketIndex = socketIndex || 0;

                                 // TODO: Revisit rpf data assignment as the InputState and other pieces are implemented. 
                                 this.nodeInstance.rpf.data[portName] = this.nodeInstance.rpf.data[portName] || [];
                                 this.nodeInstance.rpf.data[portName][socketIndex] = payload;

                                 // If we have data on each port, call fRunUpdater 
                                 // TODO: Revisit as we get updater policies at lower layers
                                 if ( Object.keys( this.nodeInstance.rpf.data ).length === numInPorts) { 
                                     return fRunUpdater.call( this.nodeInstance, updaterArgs, payload );
                                 }
                                 return;
                            }
                        }, port);
